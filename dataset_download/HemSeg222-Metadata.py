...
# Dictionary to track occurrences of patient IDs and generate unique IDs
patient_id_count = defaultdict(int)

# List to store rows for the updated JSON and CSV
updated_rows = []

# Read the JSONL file, process duplicate IDs, and create unique patient IDs
with open(hemseg_jsonl_file, "r") as f:
    for line in f:
        ...
        # Create a unique patient ID for each occurrence
        patient_id_count[patient] += 1
        new_patient = f"{patient}_dup{patient_id_count[patient]}" if patient_id_count[patient] > 1 else patient
        
        # Create rows for each slice ID
...
df_hemseg = pd.DataFrame(updated_rows)
df_hemseg.to_csv(hemseg_output_csv, index=False)
print(f"HemSeg222 slice list saved to '{hemseg_output_csv}'.")

# Check the number of unique new_patient IDs
unique_new_patients = df_hemseg["new_patient"].nunique()
print(f"Total unique new patient IDs: {unique_new_patients}")
...
total_patients = df['new_patient'].nunique()
print(f"Total number of unique patients: {total_patients}")

'''Total number of unique patients: 222'''

df_patients = df.groupby("new_patient")[["intraparenchymal", "intraventricular"]].max().reset_index()
# kepp the scan ID for each patient 
df_scan_id = df.groupby("new_patient")["scan"].first().reset_index()
df_patients = pd.merge(df_patients, df_scan_id, on="new_patient",how="left")
...
'''
Category
Intraparenchymal Only    115
Intraventricular Only    107
'''
