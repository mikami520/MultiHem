...
with open(hemseg_jsonl_file, "r") as f:
    for line in f:
        ...
        for slice_id in slice_list:
            hemseg_rows.append({"ID": slice_id, "patient": patient, "scan": scan})
...
if unique_patients == 222:
    ...
    raise ValueError(
        f"Validation failed: Expected 222 unique patients, but found {unique_patients}."
    )
...
df_rsna_flat = df_rsna.pivot_table(
    index="Participant_ID", columns="Hemorrhage_Type", values="Label", aggfunc="max"
).reset_index()
...
final_columns = ["ID"] + [
    label for label in expected_labels if label in df_rsna_flat.columns
]
...
df["Category"] = "Neither"
df.loc[(df["intraparenchymal"] == 1) & (df["intraventricular"] == 0), "Category"] = (
    "Intraparenchymal Only"
)
df.loc[(df["intraparenchymal"] == 0) & (df["intraventricular"] == 1), "Category"] = (
    "Intraventricular Only"
)
df.loc[(df["intraparenchymal"] == 1) & (df["intraventricular"] == 1), "Category"] = (
    "Both"
)
...
df_patients = (
    df.groupby("patient")[["intraparenchymal", "intraventricular"]].max().reset_index()
)
...
df_patients.loc[
    (df_patients["intraparenchymal"] == 1) & (df_patients["intraventricular"] == 0),
    "Category",
] = "Intraparenchymal Only"
df_patients.loc[
    (df_patients["intraparenchymal"] == 0) & (df_patients["intraventricular"] == 1),
    "Category",
] = "Intraventricular Only"
df_patients.loc[
    (df_patients["intraparenchymal"] == 1) & (df_patients["intraventricular"] == 1),
    "Category",
] = "Both"
